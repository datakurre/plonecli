...
from plonectl.exceptions import NoSuchValue
from plonectl.exceptions import NotInPackageError
from plonectl.registry import template_registry as reg
...
def cli(context, list_templates, versions):
    ...
    if versions:
        ...
        bobtemplates.plone: {1}\n""".format(
            plonectl_version, bobtemplates_version
        )
...
def config():
    ...
    params = ['mrbob', 'plonectl:configure_mrbob']
    echo('\nRUN: {0}'.format(' '.join(params)), fg='green', reverse=True)
